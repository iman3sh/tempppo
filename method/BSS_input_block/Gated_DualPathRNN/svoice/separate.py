...
# def save_wavs_old(estimate_source, mix_sig, lengths, filenames, out_dir, sr=8000):
...
def save_wavs(estimate_source, mix_sig, lengths, filenames, out_dir, sr=8000):
    ...
    for i, filename in enumerate(filenames):
        ...
        os.makedirs(os.path.join(out_dir, 'separated'),exist_ok=True)
        # os.makedirs(os.path.join(out_dir, 'spk2'),exist_ok=True)

        path_mix = os.path.join(out_dir,  'mix', os.path.basename(filename).strip(".wav"))
        write(mix_sig[i], path_mix + ".wav", sr=sr)
        C = flat_estimate[i].shape[0]
        # future support for wave playing
        for c in range(C):
            path_spk = os.path.join(out_dir, 'separated', os.path.basename(filename).strip(".wav")+ f"_spk{c + 1}")
            write(flat_estimate[i][c], path_spk + ".wav", sr=sr)
